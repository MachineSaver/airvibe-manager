...
for svc in \
    airvibe-manager-postgres \
    chirpstack-redis \
    mqtt-broker \
    chirpstack \
    chirpstack-gateway-bridge \
    airvibe-manager-backend \
    airvibe-manager-frontend \
    airvibe-manager-caddy
do
...
if docker exec airvibe-manager-postgres psql -U postgres -d airvibe -c '\q' &>/dev/null; then
...
CS_TABLE_COUNT=$(docker exec airvibe-manager-postgres psql -U postgres -d chirpstack -t -c \
...
for svc in airvibe-manager-postgres airvibe-manager-backend airvibe-manager-frontend airvibe-manager-caddy; do
    if container_running "$svc"; then
        pass "$svc is running"
    else
        fail "$svc is not running"
...
BACKEND_LOGS=$(docker logs airvibe-manager-backend 2>&1 || true)
...
# ---------------------------------------------------------------------------
# ThingPark mode  (NETWORK_SERVER=thingpark, COMPOSE_PROFILES=)
# ---------------------------------------------------------------------------

echo ""
echo "======================================================="
echo " THINGPARK MODE smoke test  (NETWORK_SERVER=thingpark)"
echo "======================================================="
echo "  (Backend will log MQTT errors — this is expected)"

NETWORK_SERVER=thingpark COMPOSE_PROFILES= MQTT_BROKER_URL=mqtt://localhost:9999 \
...
TP_COUNT=$(docker compose ps --format '{{.Name}}' | wc -l | tr -d ' ')
assert_eq "exactly 4 containers running" "4" "$TP_COUNT"

echo ""
echo "--- Absent services check (must NOT exist) ---"

for svc in chirpstack chirpstack-redis chirpstack-gateway-bridge mqtt-broker chirpstack-db-init; do
    if ! container_exists "$svc"; then
        pass "$svc does not exist"
    else
        fail "$svc exists but should not in ThingPark mode"
    fi
done

echo ""
echo "--- ThingPark-specific checks ---"

# Adapter log: must show thingpark adapter loaded
BACKEND_LOGS_TP=$(docker logs airvibe-manager-backend 2>&1 || true)
assert_contains "Backend logs show 'MQTT adapter: thingpark'" "MQTT adapter: thingpark" "$BACKEND_LOGS_TP"

# Caddy: generated Caddyfile must NOT contain 'tls internal' for ThingPark mode
# (domain is localhost in this test, so tls internal IS expected here — the
# ThingPark-mode ACME path is only triggered when DOMAIN is a real hostname)
# We verify the Caddyfile was generated at all (caddy entrypoint ran).
CADDY_CF=$(docker exec airvibe-manager-caddy cat /etc/caddy/Caddyfile 2>/dev/null || echo "")
assert_contains "Caddyfile was generated by entrypoint" "reverse_proxy" "$CADDY_CF"
assert_contains "Caddyfile has expected domain block" "localhost" "$CADDY_CF"

# Backend REST API should still respond
echo "  waiting for backend /health ..."
wait_http "http://localhost:4000/api/health" 60 || true
assert_http "Backend GET /health → 200 (ThingPark mode)" "http://localhost:4000/api/health" "200"
