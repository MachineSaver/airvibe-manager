...
- **openapi.js**: Full OpenAPI 3.1 specification exported as a plain JS object. Served at `GET /api/openapi.json` (raw JSON) and `GET /api/docs/` (Swagger UI via `swagger-ui-express@^4`). Both paths are auth-exempt. Covers all 28 REST endpoints with schemas, parameters, and response definitions.
