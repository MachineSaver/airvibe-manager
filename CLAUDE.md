...
- **services/FUOTAManager.js**: FUOTA firmware update state machine. Uses `networkServerClient` for Class C switching — automatic in both ChirpStack and ThingPark modes when credentials are configured; graceful no-op otherwise (manual switch required). Key behaviours: (1) `resolveIntervalLimits(ismBand, firmwareName)` returns band-keyed min/max block-send intervals (US915: 5–15 s, EU868: 60–300 s; conservative fallback for unknown bands); exported on `module.exports` for direct unit testing. (2) `_prefillConfig(devEui)` runs as the first step of `startSession` — checks `devices.metadata.config_updated_at`; if stale/absent, sets `session.state = 'config_poll'` and sends up to 3 confirmed `0x0200` config requests (fPort 22) spaced `FUOTA_CONFIG_POLL_WAIT_MS` apart, exiting early if the device responds; skipped on startup recovery. (3) `_sendInitDownlink` uses `ACK_RETRY_SCHEDULE` (13 attempts: 1–5 @ 1 min, 6–10 @ 5 min, 11–13 @ 10 min, ~60 min total) instead of a single long timeout; exhaustion calls `_failSession`. (4) Early 0x10 ACK stash: if a 0x10 arrives while state is `'initializing'` or `'config_poll'`, it is stashed on `session._earlyInitAck`; `_sendInitDownlink` checks for the stash first, consumes it, and skips the redundant downlink. (5) `_restoreClassWithRetry` retries Class A restore up to 3 times (15 s / 30 s backoff) as a non-blocking fire-and-forget. (6) `_waitForMqtt` polls `isConnected()` every 5 s — pauses until reconnected or `FUOTA_MQTT_WAIT_MS` elapses, then fails. (7) `_emitProgress` includes `startedAt` timestamp for observed-rate ETA. (8) `startSession` upserts the device row before the session INSERT to satisfy the FK constraint.
...
- **components/FUOTAManager.tsx**: FUOTA UI. Shows network-server-conditional Class C status banner (4 variants: ChirpStack/ThingPark × configured/not-configured). Fetches `/api/fuota/network-server-status` for `{ configured, type }`. Active Jobs timeline includes a dedicated **Config** step as the first node — purple/active while `config_poll` is running (shows "Config N/3"), green ✓ once the state advances. Block-send interval input min/max are live-derived from the selected firmware + ISM band (`resolveIntervalLimits`) and auto-reclamp when the firmware selection changes.
...
- FUOTA recovery + retry tests: `backend/test/fuota-recovery.test.js` — covers startup recovery, init-downlink retry schedule, early ACK stash, config-poll flow, band-specific interval limits.
- Auth middleware tests: `backend/test/auth.test.js`.
- Current total: **163 tests, 5 suites** — all passing.
