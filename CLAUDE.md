...
- **middleware/auth.js**: Exports `requireApiKey` — Bearer API key middleware. No-op when `API_KEYS_ENABLED !== "true"`. Exempt paths: `'/'` and `'/api/health'`. Returns 401 for missing/malformed Authorization header or unrecognised key. Sets `req.apiKey` on success. Network-server-agnostic (works in both ChirpStack and ThingPark modes).
